...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubMovs = onSnapshot(qMovs, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        list.push({
          ...
          descripcion: d.descripcion, unidad: d.unidad, area: d.area, notas: d.notas || '', registradoPor: d.registradoPor || '', eliminado: !!d.eliminado, eliminadoPor: d.eliminadoPor || undefined,
          prov: d.prov || undefined, fact: d.fact || undefined, fechaFact: d.fechaFact || undefined, costoUnit: d.costoUnit !== undefined ? Number(d.costoUnit) : undefined,
          costoTotal: d.costoTotal !== undefined ? Number(d.costoTotal) : undefined, aut: d.aut || undefined, tipoCompra: d.tipoCompra || undefined, recibe: d.recibe || undefined, resp: d.resp || undefined,
          horaFact: d.horaFact, impuestos: d.impuestos, totalFactura: d.totalFactura, metodoPago: d.metodoPago, tipoRecurso: d.tipoRecurso, categoriaGasto: d.categoriaGasto, marca: d.marca, modelo: d.modelo, serie: d.serie
        });
      });
      list.sort((a, b) => b.id - a.id); setMovimientos(list);
    ...
    const unsubCat = onSnapshot(qCat, (snapshot) => {
      ...
      snapshot.forEach((doc) => { const d = doc.data(); list.push({ descripcion: d.descripcion, unidad: d.unidad, tipoCompra: d.tipoCompra || undefined }); });
    ...
    const unsubUnidades = onSnapshot(qUnidades, (snapshot) => {
      const list: string[] = [];
      snapshot.forEach((doc) => { const d = doc.data(); if (d.name) list.push(d.name); });
      if (list.length === 0) seedDefaultUnidades(moduleId); else setUnidadesMaestras(list);
    }, (error) => { handleFirestoreError(error, OperationType.GET, `modules/${moduleId}/unidades`); });

    return () => { unsubMovs(); unsubCat(); unsubAreas(); unsubProvs(); unsubUnidades(); };
  }, [currentUser]);
  ...
  const seedDefaultAreas = async (moduleId: string) => {
    ...
    for (const name of defaultAreas) { const slug = name.replace(/[^a-zA-Z0-9]/g, '_').toUpperCase(); try { await setDoc(doc(db, 'modules', moduleId, 'areas', slug), { name }); } catch (err) { } }
  };

  const seedDefaultProveedores = async (moduleId: string) => {
    const defaultProvs = ['OFFICE DEPOT', 'HOME DEPOT', 'AMAZON', 'MERCADO LIBRE'];
    for (const name of defaultProvs) { const slug = name.replace(/[^a-zA-Z0-9]/g, '_').toUpperCase(); try { await setDoc(doc(db, 'modules', moduleId, 'proveedores', slug), { name }); } catch (err) { } }
  };

  const seedDefaultUnidades = async (moduleId: string) => {
    const defaultUnits = ['GALÓN', 'LITRO', 'KG', 'PIEZA', 'CAJA', 'PAQUETE', 'PAR', 'BIDÓN 20 LITROS', 'ROLLO', 'OTRO'];
    for (const name of defaultUnits) { const slug = name.replace(/[^a-zA-Z0-9]/g, '_').toUpperCase(); try { await setDoc(doc(db, 'modules', moduleId, 'unidades', slug), { name }); } catch (err) { } }
  };

  const showToast = (msg: string, type: 'success' | 'error' | 'warn' | 'info' = 'info') => { setToast({ msg, type }); setTimeout(() => { setToast(null); }, 4000); };
  ...
  const handleLogout = () => { setCurrentUser(null); setMovimientos([]); setCatalogoMaestro([]); setAreasMaestras([]); setProveedoresMaestros([]); setUnidadesMaestras([]); showToast('Sesión cerrada', 'info'); };
  const syncData = () => { if (!currentUser) return; showToast('Datos sincronizados en tiempo real', 'success'); };

  const handleSaveMovement = async (items: any[], catalogsToInsert: CatalogoItem[]) => {
    if (!currentUser) return;
    const moduleId = currentUser.module;

    for (const cand of catalogsToInsert) {
      const cleanDesc = cand.descripcion.trim().toUpperCase(); const cleanUnit = cand.unidad.trim().toUpperCase();
      const docId = `${cleanDesc}___${cleanUnit}`.replace(/[^a-zA-Z0-9]/g, '_');
      try { await setDoc(doc(db, 'modules', moduleId, 'catalogo', docId), { descripcion: cleanDesc, unidad: cleanUnit, tipoCompra: cand.tipoCompra || 'RESURTIBLE' }, { merge: true }); } catch (err) {}
    }
    ...
    for (let index = 0; index < items.length; index++) {
      const i = items[index]; const movIdStr = `${timestamp}_${index}_${Math.floor(Math.random() * 1000)}`;
      const dataToSave: any = { id: timestamp + index + Math.random(), tipo: baseTipo, fecha: i.fecha || new Date().toISOString().split('T')[0], cantidad: Math.floor(Number(i.cantidad) || 0), descripcion: i.descripcion.trim().toUpperCase(), unidad: i.unidad.trim().toUpperCase(), area: i.area.trim().toUpperCase(), notas: i.notas ? i.notas.trim().toUpperCase() : '', registradoPor: currentUser.name, eliminado: false, timestamp: timestamp };
      if (i.prov) dataToSave.prov = i.prov.trim().toUpperCase(); if (i.fact) dataToSave.fact = i.fact.trim().toUpperCase(); if (i.fechaFact) dataToSave.fechaFact = i.fechaFact; if (i.costoUnit !== undefined) dataToSave.costoUnit = Number(i.costoUnit); if (i.costoTotal !== undefined) dataToSave.costoTotal = Number(i.costoTotal); if (i.aut) dataToSave.aut = i.aut.trim().toUpperCase(); if (i.tipoCompra) dataToSave.tipoCompra = i.tipoCompra.trim().toUpperCase(); if (i.recibe) dataToSave.recibe = i.recibe.trim().toUpperCase(); if (i.resp) dataToSave.resp = i.resp.trim().toUpperCase();
      if (i.horaFact) dataToSave.horaFact = i.horaFact; if (i.impuestos !== undefined) dataToSave.impuestos = Number(i.impuestos); if (i.totalFactura !== undefined) dataToSave.totalFactura = Number(i.totalFactura); if (i.metodoPago) dataToSave.metodoPago = i.metodoPago.trim().toUpperCase(); if (i.tipoRecurso) dataToSave.tipoRecurso = i.tipoRecurso.trim().toUpperCase(); if (i.categoriaGasto) dataToSave.categoriaGasto = i.categoriaGasto.trim().toUpperCase(); if (i.marca) dataToSave.marca = i.marca.trim().toUpperCase(); if (i.modelo) dataToSave.modelo = i.modelo.trim().toUpperCase(); if (i.serie) dataToSave.serie = i.serie.trim().toUpperCase();
      try { await setDoc(doc(db, 'modules', moduleId, 'movimientos', movIdStr), dataToSave); } catch (err) {}
    }
  ...
  const handleClearHistory = async () => {
    if (!currentUser) return; const eliminados = movimientos.filter(m => m.eliminado); if (eliminados.length === 0) return;
    for (const m of eliminados) { if (m.docId) { try { await deleteDoc(doc(db, 'modules', currentUser.module, 'movimientos', m.docId)); } catch (err) {} } }
  ...
  const handleRemoveProv = async (prov: string) => { if (!currentUser) return; try { await deleteDoc(doc(db, 'modules', currentUser.module, 'proveedores', prov.trim().toUpperCase().replace(/[^a-zA-Z0-9]/g, '_'))); } catch (err) { } };
  const handleAddUnidad = async (unidad: string) => { if (!currentUser) return; const cl = unidad.trim().toUpperCase(); try { await setDoc(doc(db, 'modules', currentUser.module, 'unidades', cl.replace(/[^a-zA-Z0-9]/g, '_')), { name: cl }); } catch (err) { } };
  const handleRemoveUnidad = async (unidad: string) => { if (!currentUser) return; try { await deleteDoc(doc(db, 'modules', currentUser.module, 'unidades', unidad.trim().toUpperCase().replace(/[^a-zA-Z0-9]/g, '_'))); } catch (err) { } };

  const openAdjustmentModal = (item: { descripcion: string; unidad: string }) => { setAdjustmentModal({ isOpen: true, descripcion: item.descripcion, unidad: item.unidad, cantidad: '', tipo: 'ENTRADA', fecha: new Date().toISOString().split('T')[0], notas: '' }); };
  const closeAdjustmentModal = () => { setAdjustmentModal(prev => ({ ...prev, isOpen: false })); };

  const handleSaveAdjustment = async () => {
    if (!currentUser) return; const qty = Math.max(1, Math.floor(Number(adjustmentModal.cantidad) || 0)); const desc = adjustmentModal.descripcion.trim().toUpperCase(); const unit = adjustmentModal.unidad.trim().toUpperCase(); const notes = adjustmentModal.notas.trim().toUpperCase(); const dateStr = adjustmentModal.fecha;
    if (!dateStr || qty <= 0 || !notes) { showToast('Faltan datos', 'error'); return; }
    if (adjustmentModal.tipo === 'SALIDA') { const prod = getInventarioActual(movimientos, catalogoMaestro).find(i => i.descripcion === desc && i.unidad === unit); if (!prod || prod.actual < qty) { showToast('Sin stock', 'error'); return; } }
    const movIdStr = `${Date.now()}_adjust_${Math.floor(Math.random() * 1000)}`;
    try { await setDoc(doc(db, 'modules', currentUser.module, 'movimientos', movIdStr), { id: Date.now(), tipo: adjustmentModal.tipo, fecha: dateStr, cantidad: qty, descripcion: desc, unidad: unit, area: 'AJUSTE MANUAL', notas: `[AUDITORÍA]: ${notes}`, registradoPor: currentUser.name, eliminado: false, timestamp: Date.now() }); closeAdjustmentModal(); showToast('Ajuste guardado', 'success'); } catch (err) { }
  };
    ...
    <div id="app" className="min-h-screen flex flex-col bg-[#f5f3ee] text-[#1a1814] font-sans">
      <header className="bg-[#1a1814] text-white px-6 py-3 flex items-center justify-between sticky top-0 z-50 shadow-md">
        <div className="header-left flex items-center gap-4">
          <span className="header-logo font-mono text-xs md:text-sm tracking-[2px] font-bold uppercase">CCU · <span className={isContabilidad ? 'text-amber-400' : 'text-sky-400'}>{currentUser.module}</span></span>
        </div>
        <div className="flex items-center gap-3">
          <div className="user-chip flex items-center gap-2 bg-white/10 px-3 py-1 rounded-full text-xs uppercase tracking-wider font-semibold">
            <span className={`w-2.5 h-2.5 rounded-full ${userDotColor}`} /><span id="user-name-display">{currentUser.name}</span><span className="text-white/45 text-[10px] md:inline hidden">({roleLabel})</span>
          </div>
          <button onClick={handleLogout} className="flex items-center gap-1.5 bg-white/15 hover:bg-white/25 text-white py-1.5 px-3 border border-white/25 rounded-md text-xs font-bold transition-colors cursor-pointer"><LogOut size={13} /> Salir</button>
        </div>
      </header>
      ...
      {isContabilidad && (
        <nav id="main-nav" className="bg-white border-b border-[#ddd9d0] px-6 py-0 flex gap-1.5 overflow-x-auto scrollbar-none print:hidden">
          ...
          <button onClick={() => setActiveTab('inventario')} className={`nav-btn py-4 px-4 text-xs font-semibold flex items-center gap-2 border-b-2 transition-all cursor-pointer whitespace-nowrap ${activeTab === 'inventario' ? 'border-amber-600 text-amber-700 font-bold' : 'border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-50'}`}><BookOpen size={15} /> Libro Contable / Activos</button>
          <button onClick={() => setActiveTab('basedatos')} className={`nav-btn py-4 px-4 text-xs font-semibold flex items-center gap-2 border-b-2 transition-all cursor-pointer whitespace-nowrap ${activeTab === 'basedatos' ? 'border-amber-600 text-amber-700 font-bold' : 'border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-50'}`}><History size={15} /> Historial Financiero</button>
          <button onClick={() => setActiveTab('areas')} className={`nav-btn py-4 px-4 text-xs font-semibold flex items-center gap-2 border-b-2 transition-all cursor-pointer whitespace-nowrap ${activeTab === 'areas' ? 'border-amber-600 text-amber-700 font-bold' : 'border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-50'}`}><MapPin size={15} /> Proveedores y Áreas CCU</button>
          {(isAdmin || isSup) && (<button onClick={() => setActiveTab('reportes')} className={`nav-btn py-4 px-4 text-xs font-semibold flex items-center gap-2 border-b-2 transition-all cursor-pointer whitespace-nowrap ${activeTab === 'reportes' ? 'border-amber-600 text-amber-700 font-bold' : 'border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-50'}`}><Calculator size={15} /> Reportes y Analíticas</button>)}
        </nav>
      )}
